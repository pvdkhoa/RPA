...
| **RunWorkflowTool** | Run or debug the workflow for testing | `command`, `filePath`, `inputArguments?`, `inputVariables?`, `logLevel?` |
...
- **[debugging.md](./references/debugging.md)** — Full `RunWorkflowTool` debugging reference: all debug commands (`StartDebugging`, `StepOver`, `StepInto`, `TestActivity`, `StartDebuggingFromHere`, etc.), input variables vs arguments, output format, and common debugging workflows
...
- **[debugging.md](./references/debugging.md)** — Interactive debugging with `RunWorkflowTool`: all debug commands (breakpoints, stepping, `TestActivity`, `StartDebuggingFromHere`), input variables vs arguments, output format, exception investigation, and runtime state inspection. Read when you need to go beyond static validation (`GetErrorsTool`) to diagnose runtime issues
...
- Use `RunWorkflowTool` for runtime validation if static checks pass. For the full debug command reference (breakpoints, stepping, `TestActivity`, `StartDebuggingFromHere`, exception handling, output format), see **[debugging.md](./references/debugging.md)**
- For complex runtime issues, use **interactive debugging** (`command: StartDebugging`) to step through execution, inspect variables at each point, and pinpoint exactly where logic diverges from expectations
...
- [ ] Smoke test with `RunWorkflowTool` considered/executed (if workflow is safe to run and/or debug). For advanced debugging (breakpoints, stepping, isolated activity testing), see [debugging.md](./references/debugging.md)
